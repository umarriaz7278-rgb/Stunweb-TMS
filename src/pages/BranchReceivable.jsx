...
export default function BranchReceivable({ branchName }) {
  ...
  const handleDeleteReceived = async (id) => {
    await supabase.from('branch_profit_received').delete().eq('id', id);
    fetchReceived();
  };

  const calcProfit = (ch) => parseFloat(ch.branch_deposit) || 0;
