...
export default function BranchDeliveryReport({ branchName }) {
  ...
  const handleDeleteReceived = async (id) => {
    await supabase.from('branch_commission_received').delete().eq('id', id);
    fetchReceived();
  };
  ...
  const totalCommissionDue = monthChallans.reduce((s, ch) => s + (parseFloat(ch.commission_deduction) || 0), 0);
  const totalReceived = monthReceived.reduce((s, r) => s + (parseFloat(r.amount) || 0), 0);
  const balance = totalCommissionDue - totalReceived;

  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '6px', flexWrap: 'wrap', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <DollarSign size={26} color="var(--primary-color)" />
          <h1 className="page-title" style={{ marginBottom: 0 }}>Delivery Report — {branchName}</h1>
        </div>
      </div>
      <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '20px' }}>
        Track {branchName} branch challan commissions and received payments.
      </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '14px', marginBottom: '22px' }}>
        {[
          { label: 'Total Commission Due', value: totalCommissionDue, color: '#6366f1' },
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '18px', borderBottom: '2px solid var(--border)' }}>
        {[['ledger', 'Commission Ledger (Challans)'], ['received', 'Amount Received']].map(([key, label]) => (
      ...
      {activeTab === 'ledger' && (
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          {loadingChallans ? (
            ...
            <div style={{ padding: '32px', textAlign: 'center', color: 'var(--text-muted)' }}>No {branchName} challans found for {filterMonth}.</div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.88rem' }}>
                <thead>
                  <tr style={{ background: 'var(--bg-secondary)', borderBottom: '2px solid var(--border)' }}>
                    {['#', 'Challan No', 'Date', 'Vehicle No', 'Commission Amount'].map(h => (
                ...
                <tbody>
                  {monthChallans.map((ch, i) => (
                    <tr key={ch.id} style={{ borderBottom: '1px solid var(--border)' }}
                      ...
                      <td style={{ padding: '10px 14px', fontWeight: 700, color: '#6366f1' }}>Rs. {(parseFloat(ch.commission_deduction) || 0).toLocaleString('en-PK')}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr style={{ background: 'var(--bg-secondary)', borderTop: '2px solid var(--border)', fontWeight: 700 }}>
                    ...
                    <td style={{ padding: '10px 14px', color: '#6366f1', fontSize: '0.95rem' }}>Rs. {totalCommissionDue.toLocaleString('en-PK')}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
        ...
        <div>
              ...
              <form onSubmit={handleAddReceived}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '16px' }}>
                  ...
                  <div className="form-group">
                    ...
                    <input type="text" name="description" value={form.description} onChange={handleFormChange} placeholder="e.g. Commission payment" />
                  </div>
                  <div className="form-group">
          ...
          <div className="card" style={{ marginTop: '16px', padding: '16px 20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.9rem' }}>
              <span>Total Commission Due ({filterMonth}):</span>
              <strong style={{ color: '#6366f1' }}>Rs. {totalCommissionDue.toLocaleString('en-PK')}</strong>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.9rem' }}>
              <span>Total Received:</span>
              <strong style={{ color: '#10b981' }}>Rs. {totalReceived.toLocaleString('en-PK')}</strong>
            </div>
