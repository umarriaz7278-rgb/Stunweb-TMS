...
export default function BrokerReceivableRawalpindi() {
  const [activeTab, setActiveTab] = useState('ledger');
  const [filterMonth, setFilterMonth] = useState(new Date().toISOString().slice(0, 7));

  // Challan ledger (from challans with Rawalpindi bilties)
  ...
  const [closedMonths, setClosedMonths] = useState(() => {
    const s = localStorage.getItem('broker_receivable_rawalpindi_closed');
  ...
  const toggleClose = () => {
    ...
    localStorage.setItem('broker_receivable_rawalpindi_closed', JSON.stringify(updated));
  };
  ...
  async function fetchChallans() {
    ...
    const rawalpindiChallanIds = (cbData || [])
      .filter(cb => {
        ...
        return bName === 'rawalpindi' || bDest === 'rawalpindi' || bName.startsWith('rawalpindi') || bDest.startsWith('rawalpindi');
      })
  ...
  async function fetchReceived() {
    ...
    const { data, error } = await supabase
      .from('broker_received_rawalpindi')
      .select('*')
      .order('id', { ascending: false });
  ...
  const handleAddReceived = async (e) => {
    ...
    setSaving(true);
    const { error } = await supabase.from('broker_received_rawalpindi').insert([{
      date: form.date,
  ...
  const handleDeleteReceived = async (id) => {
    await supabase.from('broker_received_rawalpindi').delete().eq('id', id);
    fetchReceived();
  };
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '6px', flexWrap: 'wrap', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <h1 className="page-title" style={{ marginBottom: 0 }}>A/C Receivable Broker (Rawalpindi)</h1>
        </div>
      </div>
      <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '20px' }}>
        Track Rawalpindi branch challan broker receivable and received payments.
      </p>
